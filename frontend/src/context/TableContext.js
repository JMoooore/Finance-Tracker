...
export const TableProvider = ({ children }) => {
    ...
    const postNewAccount = async (userId, obj) =>{
        const response = await axios.post('http://localhost:3001/accounts/forUser1', obj)
        return response.id
    }

    const postNewPayee = async (userId, obj)=>{
        const response = await axios.post('http://localhost:3001/payees/forUser1', obj)
        return response[0].id
    }

    const postNewCategory = async (userId, obj) =>{
        const response = await axios.post('http://localhost:3001/categories/forUser1', obj)
        return response[0].id
    ...
    const deleteTransaction = async (oldTransaction) =>{
        ...
        const response = await axios.delete(`http://localhost:3001/transactions/${transaction_id}`)
        if(response.status !== 200) return 
        const newArr = userData.transactions.filter((trans)=> trans.transaction_id != transaction_id)
        setUserData({...userData, transactions: newArr})

    }

    const postNewTrans = async (userId, obj, newData) =>{
        const response = await axios.post('http://localhost:3001/transactions/forUser1', obj)
    ...
    const addTransaction = async (userInput) =>{
        ...
        data.payee_id = userData.payees.find((payee)=> payee.name == userInput.payee_name).id
        ...
        data.category_id = userData.categories.find((category)=> category.name == userInput.category_name).id
        ...
        data.account_id = userData.accounts.find((account)=> account.name == userInput.account_name).id
        // if(data.payee_id === undefined || data.payee_id === null){
        //     let accountObj = {}
        ...
        // }
        const response = await axios.post('http://localhost:3001/transactions/forUser1', data)
