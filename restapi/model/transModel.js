...
transModel.editTransaction = async (id, body)=>{
    ...
    const newData = await db.query('UPDATE transactions SET category_id=$1, payee_id=$2, account_id=$3, outflow=$4, inflow=$5, note=$6 WHERE id=$7 RETURNING *', [newTrans.category_id, newTrans.payee_id, newTrans.account_id, newTrans.outflow, newTrans.inflow, newTrans.note, id])
    return newData.rows
}
